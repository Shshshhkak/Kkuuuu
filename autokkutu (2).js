/* ===================================================================
 * AutoKkutu Web - kkutu.co.kr 전용 (v1.2: WS 끊김 1014 해결판)
 * - 사전 로드/검색을 Web Worker 로 완전 분리 (메인 쓰레드 ws ping 보호)
 * - 사전 자동 로드 비활성화 (버튼 클릭 시에만)
 * - localStorage 큰 객체 직렬화 제거
 * - 폴링 간격 350ms 로 완화
 * =================================================================== */
(function () {
  "use strict";
  ...
  const workerSrc = `
    ...
    function buildIndex(){
      INDEX = new Map(); STARTS = new Set();
      for (let i=0;i<DICT.length;i++){
        const w = DICT[i]; const c = w[0];
        STARTS.add(c);
        let arr = INDEX.get(c);
    ...
    self.onmessage = (e) => {
      const m = e.data || {};
      if (m.cmd === "load") {
        fetch(m.url, { cache: m.force ? "reload" : "default" })
          .then(r => r.text())
          .then(txt => {
            DICT = txt.split(/\\r?\\n/).map(s=>s.trim()).filter(w=>w.length>=2);
            buildIndex();
            self.postMessage({ ev:"loaded", count: DICT.length });
          })
          .catch(err => self.postMessage({ ev:"error", msg: String(err && err.message || err) }));
      }
      else if (m.cmd === "search") {
        if (!ready) { self.postMessage({ev:"searched", ch:m.ch, items:[], used:m.used}); return; }
  ...
  function buildPanel() {
    ...
    head.innerHTML = '<div class="ak-title">단어 도우미 <small>v1.2</small></div>';
    ...
    dictRow.appendChild(dlBtn); dictRow.appendChild(reBtn);
  ...
  updateStatus();
  log("v1.2 로드. '사전 로드' 버튼을 눌러 단어를 가져오세요.");
})();
